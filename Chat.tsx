...
import { ChatMessage, Language, TutorStyle, ModelId, GroundingChunk, SubscriptionTier, FeatureId, TtsProvider, Emotion } from './types';
...
const Chat: React.FC<ChatProps> = ({ nativeLanguage, learningLanguage, setNativeLanguage, setLearningLanguage, incrementChatCount, subscriptionTier, setActiveFeature }) => {
  ...
  // New state for emotion and STT language
  ...
  const { isRecording, isTranscribing, transcript, error: sttError, startRecording, stopRecording } = useSpeechToText();

  const chatRef = useRef<GeminiChat | null>(null);
  const endOfMessagesRef = useRef<HTMLDivElement>(null);

  useEffect(() => { endOfMessagesRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [chatHistory, isLoading]);
  useEffect(() => { chatRef.current = null; setChatHistory([]); }, [nativeLanguage, learningLanguage, tutorStyle, selectedModel, isGrounded, emotionMode]);
  ...
  const handleMicClick = () => {
    if (isRecording) {
        ...
        const lang = sttLanguageCode === 'auto' ? undefined : languages.find(l => l.code === sttLanguageCode);
        startRecording(lang);
    }
  };
  ...
  const generateChatSuggestions = useCallback(async (): Promise<string[]> => {
      ...
      try {
          ...
          if (e?.toString().includes('quota')) { throw new Error("API quota exceeded."); }
          return [];
      }
  ...
  const handleSendMessage = async (e?: FormEvent<HTMLFormElement>, messageOverride?: string) => {
    ...
    try {
      if (!chatRef.current) {
        ...
        if (['gemini-2.5-pro', 'gpt-5', 'claude-opus-4.1', 'claude-opus-4', 'claude-opus-3', 'auto'].includes(selectedModel)) {
            config.thinkingConfig = { thinkingBudget: 32768 };
        }
        if (isGrounded && modelName !== 'gemini-flash-lite-latest') { config.tools = [{ googleSearch: {} }]; }
        chatRef.current = ai.chats.create({ model: modelName, config });
      }
      ...
      for await (const chunk of stream) {
        ...
        if (chunk.candidates?.[0]?.groundingMetadata?.groundingChunks) {
            modelResponseSources = chunk.candidates[0].groundingMetadata.groundingChunks;
        }
        setChatHistory(prev => prev.map(m => m.id === modelMessageId ? { ...m, parts: [{ text: modelResponseText }], sources: modelResponseSources } : m));
      }
    } catch (err: any) {
      console.error(err);
      let errorMessage = err?.toString().includes('quota') ? "API quota exceeded. Please check your plan or try again tomorrow." : 'Sorry, something went wrong. Please try again.';
  ...
  const handlePlayTTS = async (messageId: string, text: string) => {
    ...
    const parseEmotionFromText = (responseText: string): Emotion | null => {
        const match = responseText.match(/\*\*Emotion:\*\*\s*_([a-zA-Z]+)_/);
        return match ? match[1].toLowerCase() as Emotion : null;
    };

    const emotionToVoiceMap: Record<Emotion, string> = {
        neutral: 'Kore',
        warm: 'Zephyr',
        encouraging: 'Puck',
        excited: 'Puck', // No directly "excited" voice, Puck is lively
        calm: 'Charon',
        serious: 'Fenrir',
        playful: 'Zephyr',
        empathetic: 'Kore',
    ...
    if (emotionMode === 'Auto') {
        emotionToUse = parseEmotionFromText(text) || 'neutral';
    }
    ...
    try {
        ...
        console.error(err);
        setError(err?.toString().includes('quota') ? 'Could not generate speech: API quota exceeded.' : 'Failed to generate speech.');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-8 h-full flex flex-col bg-background-primary text-text-primary">
      <PageHeader title="Practice Conversation" description={`Practice your ${learningLanguage.name} with your AI partner, Lumi.`} nativeLanguage={nativeLanguage} learningLanguage={learningLanguage} setNativeLanguage={setNativeLanguage} setLearningLanguage={setLearningLanguage} />
              ...
              <div className={`max-w-xl px-5 py-3 rounded-2xl shadow-md ${message.role === 'user' ? 'bg-accent-primary text-background-primary' : 'bg-background-tertiary text-text-primary'}`}>
               ...
               {message.role === 'model' && (
                  <div className="w-full max-w-xl">
                     {message.parts[0].text && <MessageToolbar message={message} isLastModelMessage={index === chatHistory.length - 1} onPlayTTS={handlePlayTTS} onRegenerate={handleRegenerate} ttsLoadingMessageId={ttsLoadingMessageId} />}
        ...
        <div className="border-t border-background-tertiary/50 p-4 bg-background-secondary">
          <form onSubmit={handleSendMessage} className="flex items-end space-x-2 sm:space-x-4">
              <div className="relative">
                <button type="button" onClick={() => setIsSettingsOpen(true)} className="p-2 text-text-secondary hover:text-accent-primary transition-colors" aria-label="Tutor Style Settings"><SettingsIcon /></button>
                <ChatSettings isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} currentStyle={tutorStyle} onStyleChange={setTutorStyle} currentModel={selectedModel} onModelChange={setSelectedModel} isGrounded={isGrounded} onGroundedChange={setIsGrounded} ttsProvider={ttsProvider} onTtsProviderChange={setTtsProvider} subscriptionTier={subscriptionTier} setActiveFeature={setActiveFeature} emotionMode={emotionMode} onEmotionModeChange={setEmotionMode} manualEmotion={manualEmotion} onManualEmotionChange={setManualEmotion} sttLanguageCode={sttLanguageCode} onSttLanguageCodeChange={setSttLanguageCode} nativeLanguage={nativeLanguage} learningLanguage={learningLanguage} />
              </div>
            <div className="flex-1 flex flex-col gap-2">
                <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(e as any); } }} aria-label={`Type or use the mic to speak in ${learningLanguage.name}`} placeholder={`Type or use the mic to speak in ${learningLanguage.name}...`} className="w-full bg-background-tertiary rounded-lg p-3 text-text-primary placeholder-text-secondary focus:ring-2 focus:ring-accent-primary focus:outline-none resize-none transition-all duration-200" rows={2} disabled={isLoading || isRecording || isTranscribing} />
                {sttError && <p role="alert" className="text-xs text-red-400 px-1">{sttError}</p>}
                {!isLoading && ( <SmartSuggestions generateSuggestions={generateChatSuggestions} onSuggestionClick={handleSuggestionClick} isDisabled={isLoading || isRecording || isTranscribing} /> )}
            </div>
            <button type="button" onClick={handleMicClick} disabled={isLoading || isTranscribing} className={`p-3 rounded-lg transition-colors duration-200 flex items-center justify-center self-stretch ${isRecording ? 'bg-red-500/80 text-white animate-pulse' : 'bg-background-tertiary text-text-secondary hover:bg-background-tertiary/70'}`} aria-label={isRecording ? 'Stop recording' : 'Start recording'}>
                  {isTranscribing ? <TranscribingSpinner /> : isRecording ? <StopIcon /> : <MicIcon />}
            </button>
            <button type="submit" disabled={isLoading || !userInput.trim() || isRecording || isTranscribing} className="bg-accent-primary text-white font-semibold py-3 px-5 rounded-lg hover:bg-accent-primary-dark disabled:bg-background-tertiary disabled:cursor-not-allowed transition-colors duration-200 flex items-center justify-center self-stretch" aria-label="Send message">
...
const MessageToolbar: React.FC<MessageToolbarProps> = ({ message, isLastModelMessage, onPlayTTS, onRegenerate, ttsLoadingMessageId }) => {
    ...
    return ( <div className="flex items-center gap-1 mt-2 ml-2"> <button onClick={handleCopy} aria-label="Copy message text" className="p-1.5 rounded-md text-text-secondary hover:bg-background-tertiary hover:text-text-primary transition-colors"> {isCopied ? <CheckIcon /> : <CopyIcon />} </button> <button onClick={() => onPlayTTS(message.id, message.parts[0].text)} aria-label="Read message aloud" disabled={!!ttsLoadingMessageId} className="p-1.5 rounded-md text-text-secondary hover:bg-background-tertiary hover:text-text-primary transition-colors disabled:opacity-50"> {ttsLoadingMessageId === message.id ? <Spinner/> : <SpeakerIcon />} </button> {isLastModelMessage && ( <button onClick={onRegenerate} aria-label="Regenerate response" className="p-1.5 rounded-md text-text-secondary hover:bg-background-tertiary hover:text-text-primary transition-colors"> <RefreshIcon /> </button> )} </div> );
};
